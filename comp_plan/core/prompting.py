# comp_plan/core/prompting.py - ICM analysis prompt builder + LLM call
import os
import json
import requests
...
def call_llm(prompt: str, temperature: Optional[float] = None) -> str:
    ...
    Tries the pluggable LLM adapter first (supports MicroLLM or any
    custom backend). Falls back to the built-in OpenAI/Ollama/stub backends.
    """
    try:
        from comp_plan.llm.llm_adapter import call_llm as _adapter_call
        return _adapter_call(prompt, temperature)
    except ImportError:
        # No adapter available — use legacy backends
        global TEMPERATURE
