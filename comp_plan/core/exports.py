# comp_plan/core/exports.py - Analysis export formats
import pandas as pd
...
def simple_html_summary(analysis: dict) -> str:
    """Generate HTML risk summary."""
    risks = "".join(
        f"<li><b>{r.get('title')}</b> – {r.get('severity')}: {r.get('detail')}</li>"
...
def to_pdf(html_str: str, outpath: Path):
    """Export as HTML file (PDF generation deferred to avoid system deps)."""
